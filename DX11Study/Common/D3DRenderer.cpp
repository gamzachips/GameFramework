...
void D3DRenderer::CreateDepthStencilView()
{
	...
	HRESULT hr = _device->CreateTexture2D(&descDepth, nullptr, &textureDepthStencil);
	assert(SUCCEEDED(hr));
	...
	hr = _device->CreateDepthStencilView(textureDepthStencil, &descDSV, _depthStencilView.GetAddressOf());
	assert(SUCCEEDED(hr));
