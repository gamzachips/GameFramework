...
void Material::ProcessMaterial(ComPtr<ID3D11Device> device, const aiMaterial* material, const aiScene* scene)
{
	...
	if (material->GetTexture(aiTextureType_OPACITY, 0, &texturePath) == AI_SUCCESS)
	{
		...
		if (embeddedTexture != nullptr)
			Texture::SaveEmbeddedTexture(embeddedTexture);
		_opacity = new Texture(device, Texture::GetTextureName(texturePath));
		_materialProperties.hasOpacityMap = 1;
	}
	if (material->GetTexture(aiTextureType_EMISSIVE, 0, &texturePath) == AI_SUCCESS)
	{
		...
		if (embeddedTexture != nullptr)
			Texture::SaveEmbeddedTexture(embeddedTexture);
		_emissive = new Texture(device, Texture::GetTextureName(texturePath));
		_materialProperties.hasEmissiveMap = 1;
	}
	if (material->GetTexture(aiTextureType_SHININESS, 0, &texturePath) == AI_SUCCESS)
	{
		...
		if (embeddedTexture != nullptr)
			Texture::SaveEmbeddedTexture(embeddedTexture);
		_roughness = new Texture(device, Texture::GetTextureName(texturePath));
		_materialProperties.hasRoughnessMap = 1;
	}
	if (material->GetTexture(aiTextureType_METALNESS, 0, &texturePath) == AI_SUCCESS)
	{
		...
		if (embeddedTexture != nullptr)
			Texture::SaveEmbeddedTexture(embeddedTexture);
		_metalness = new Texture(device, Texture::GetTextureName(texturePath));
		_materialProperties.hasMetalnessMap = 1;
	}
}
